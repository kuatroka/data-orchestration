...
async def fetch_yahoo_prices(tickers):
...
async def fetch_data_av(tickers):
                ...
                if is_internet_connected():
                    try:          
                        for ticker in tickers[ticker_idx_from:]:   
                            ...
                            print('Requests left: ',rate_limit_remaining_requests)
                            # print(start_text)
    ...
    print(f"Download took: {(end-start)/60 :0.2f} min\n")


async def all_coroutines():
    start = time.perf_counter()
    coroutines = [
                fetch_data_av(tickers),
                fetch_yahoo_prices(tickers)
                ]
